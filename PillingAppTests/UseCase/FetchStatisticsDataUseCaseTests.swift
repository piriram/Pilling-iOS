...
final class FetchStatisticsDataUseCaseTests: XCTestCase {
    ...
    func test_복용률_100퍼센트_계산() throws {
        ...
        let cycle = Cycle(id: UUID(), startDate: startDate, endDate: nil, records: records)
        mockRepository.cycles = [cycle]
        mockUserDefaultsManager.pillInfo = PillInfo(name: "테스트약", activeDays: 3, restDays: 0)
    ...
    func test_정시복용_지연복용_미복용_각각_분류() throws {
        ...
        let cycle = Cycle(id: UUID(), startDate: startDate, endDate: nil, records: records)
        mockRepository.cycles = [cycle]
        mockUserDefaultsManager.pillInfo = PillInfo(name: "테스트약", activeDays: 4, restDays: 0)
    ...
    func test_휴약일_제외하고_통계_계산() throws {
        ...
        let cycle = Cycle(id: UUID(), startDate: startDate, endDate: nil, records: records)
        mockRepository.cycles = [cycle]
        mockUserDefaultsManager.pillInfo = PillInfo(name: "테스트약", activeDays: 3, restDays: 2)
    ...
    func test_빈_사이클_isEmpty_true_반환() throws {
        ...
        let cycle = Cycle(id: UUID(), startDate: startDate, endDate: nil, records: records)
        mockRepository.cycles = [cycle]
        mockUserDefaultsManager.pillInfo = PillInfo(name: "테스트약", activeDays: 2, restDays: 0)
